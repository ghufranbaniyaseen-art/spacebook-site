 ...
 * 2) بتنركّب القوائم المنسدلة بالعمودين O و P (نفس قاعدة الصف 2)
...
var TEMPLATE_ROW = 2;   // أول كتاب: منه بناخد قواعد القوائم
...
function onEdit(e) {
  ...
  // قواعد القوائم من الصف 2، ولو مش موجودة بنبنيها
  var rules = sheet.getRange(TEMPLATE_ROW, COL_STATUS, 1, 2).getDataValidations()[0];
  if (!rules[0]) rules[0] = listRule_(['متوفر', 'غير متوفر']);
  if (!rules[1]) rules[1] = listRule_(['نشر', 'متوقف']);
  ...
  newRows.forEach(function (row) {
    ...
    np.setDataValidations([[null, rules[0], rules[1]]]);
